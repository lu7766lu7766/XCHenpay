                            ...
                            <div class="col-sm-9 p-t-7">
                                <div class="stored-txt">
                                    <span>最低储值:<i class="text-red price">
                                        {{ data.minimum_amount | numFormat('0,0') }}</i></span>
                                    <span>最高储值:<i class="text-red price">
                                        {{ data.maximum_amount | numFormat('0,0') }}</i></span>
                                </div>
                            </div>
                        </div>
                        <div class="form-group row">
...
<script>
    ...
    export default {
        ...
        mounted() {
            this.$root.$on('companyAccountInfo.show', data => {
                this.data = data
                $(this.$refs.modal).modal('show')
            })
        },
        destroyed() {
            this.$root.$off('companyAccountInfo.show')
        }
    }
</script>
